    ...
    internal sealed class Program
    {
        ...
        private static bool CheckProcessUnique()
        {
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
            {
                ...
                var mutex = new Mutex(true, AppName, out mutexCreated);
                if (!mutexCreated)
                {
            ...
            {
                string lockPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), AboutInfo.AppName,
                    ".lock");
