...
public partial class MainWindow : ReactiveWindow<MainWindowViewModel>
{
    ...
    {
        ILauncher launcher = GetTopLevel(this)!.Launcher;
    ...
    private async void OnOpenProjButtonClicked(object? sender, RoutedEventArgs e)
    {
        if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
        {
            Uri url = new(_projectsWebsiteUrl);
            await _openUri(url);
        }
        else
        {
    ...
    private async void AutoAlterScreenWidth()
    {
        ...
        HistoryItemViewModel? selectedItem = HistoryComboBox.SelectedItem as HistoryItemViewModel;
        ...
        IInteractionContext<SettingWindowViewModel, MyConfigModel> interaction)
    {
        ...
        MyConfigModel? result = await settingWindow.ShowDialog<MyConfigModel?>(this);
    ...
    private void RefreshThemeColor()
    {
        ...
        MainBorder.Material.TintColor = App.Current.ColorCollection[tempConfig.ColorIndex].Color;
